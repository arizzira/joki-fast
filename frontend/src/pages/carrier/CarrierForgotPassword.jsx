...
export default function CarrierForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axiosInstance.post('/auth/carrier/forgot-password', { email });
        ...
        <div className="min-h-screen flex bg-slate-50 lg:bg-white font-sans relative overflow-hidden">
            ...
            <div className="relative z-10 w-full lg:w-1/2 flex flex-col justify-center px-4 sm:px-8 lg:px-16 xl:px-24 py-12">
                ...
                >
                    ...
                    <Link to="/carrier/login" className="inline-flex items-center gap-2 text-sm font-medium text-slate-400 hover:text-emerald-600 transition-colors mb-8">
                        <ArrowLeft className="w-4 h-4" />
                        Kembali ke Login Worker
                    </Link>
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-slate-500 text-base leading-relaxed">
                            Jangan panik. Masukkan alamat email worker yang terdaftar, dan kami akan mengirimkan instruksi untuk mereset password Anda.
                        </p>
                    </div>
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        {success ? (
                            <div className="text-center py-6">
                                ...
                                <Link to="/carrier/login" className="inline-flex items-center gap-2 text-sm font-semibold text-emerald-600 hover:text-emerald-700 mt-6 transition-colors">
                                    <ArrowLeft className="w-4 h-4" /> Kembali ke Login Worker
                                </Link>
                            </div>
                        ) : (
                            <>
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold rounded-xl shadow-lg shadow-emerald-600/20 transition-all duration-200 flex items-center justify-center gap-2"
                                >
                                    {loading ? (
            ...
            <div className="hidden lg:flex w-1/2 relative overflow-hidden items-center justify-center bg-slate-900 border-l border-slate-800">
                <div className="absolute inset-0 z-0 bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900">
                    <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-emerald-600/20 rounded-full mix-blend-screen filter blur-3xl opacity-40 pointer-events-none"></div>
                    <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-500/10 rounded-full mix-blend-screen filter blur-3xl opacity-40 pointer-events-none"></div>
                </div>
                ...
                <div className="relative z-10 w-full max-w-lg p-12 text-white">
                    ...
                    <p className="text-lg text-slate-300 leading-relaxed max-w-md">
                        Kami memastikan data dan privasi setiap worker terlindungi dengan baik. Jangan ragu untuk menghubungi admin jika Anda mengalami kesulitan mengakses akun worker Anda.
                    </p>
                </div>
            </div>
        </div>
